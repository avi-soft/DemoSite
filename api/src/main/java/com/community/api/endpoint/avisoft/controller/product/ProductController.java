...
//            ExpirationDate and GoLiveDate VALIDATION
            ...
            if (addProductDto.getProductState() != null) {
                ...
                if (customProductState == null) {
                    return new ResponseEntity<>("No productState exist with this id", HttpStatus.INTERNAL_SERVER_ERROR);
                }
                ...
                if (role.equals(Constant.SERVICE_PROVIDER) && customProduct.getProductState().getProductState().equals(Constant.PRODUCT_STATE_NEW) && customProductState.getProductState().equals(Constant.PRODUCT_STATE_APPROVED)) {
                    ...
                    return new ResponseEntity<>("Already in Approved State", HttpStatus.INTERNAL_SERVER_ERROR);
                } else if (role.equals(Constant.SERVICE_PROVIDER)) {
                    return new ResponseEntity<>("ServiceProvider can only modify productState from new to approve", HttpStatus.INTERNAL_SERVER_ERROR);
                }
                customProduct.setProductState(customProductState);
            }
...
//                    if(addProductDto.getBornAfter()==null && addProductDto.getBornAfter() == null && addProductDto.getFee() == null && addProductDto.getPost() == null){
//                        return new ResponseEntity<>("Nothing to update in reserveCategoryData", HttpStatus.INTERNAL_SERVER_ERROR);
//                    }
//
...
//                        dateFormat.parse(dateFormat.format(addProductDto.getBornAfter()));
//                    }
//                    if (addProductDto.getFee() != null && addProductDto.getFee() <= 0) {
//                        return new ResponseEntity<>("Fee cannot be less than or equal to zero", HttpStatus.INTERNAL_SERVER_ERROR);
//                    }
//                    if (addProductDto.getPost() != null && addProductDto.getPost() <= 0) {
//                        return new ResponseEntity<>("Number of Post cannot be less than or equal to zero", HttpStatus.INTERNAL_SERVER_ERROR);
//                    }
//                    // save the data.
//
...
//                        return new ResponseEntity<>("Fee, BornBefore and bornAfter are mandatory for new ReserveCategory Data", HttpStatus.INTERNAL_SERVER_ERROR);
//                    }
